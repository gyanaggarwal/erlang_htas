...
handle_event({state, {Module, Msg, StateData=#eh_system_state{}}}, State) ->
  ...
  io:fwrite("[~p] ~p node_id=~p, repl_ring=~p, successor=~p, timestamp=~p, last_msg_succ=~p, pre_msg_data=~p, msg_data=~p, update_initiated=~p, update_completed=~p, pred_pre_update=~p, pred_update=~p~n",
            [Module,                                           Msg,
             NodeId,                                           ReplRing,
             Successor,                                        Timestamp, LastMsgSucc,
             PreMsgData,                                       MsgData,
